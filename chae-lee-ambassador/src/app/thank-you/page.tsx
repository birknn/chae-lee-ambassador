...
const INSTAGRAM = process.env.NEXT_PUBLIC_INSTAGRAM_HANDLE ?? "@ammg.studio";
      ...
      >
        <div className="relative mb-8">
          <div className="w-24 h-24 rounded-full bg-gradient-to-br from-kb-petal to-kb-rose flex items-center justify-center shadow-blush-glow">
            <svg className="w-10 h-10 text-kb-blush" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
            </svg>
          </div>
          {["top-0 -right-2", "bottom-1 -left-3", "top-2 -left-1"].map((pos, i) => (
            <span key={i} className={`absolute ${pos} text-kb-gold animate-pulse`} style={{ animationDelay: `${i * 0.4}s`, fontSize: i === 0 ? "18px" : "12px" }}>✦</span>
          ))}
        </div>
        ...
        <div className="w-full glass-card rounded-3xl p-7 mb-8 shadow-card-luxury text-left">
          ...
          <div className="flex items-start gap-3 py-3 border-t border-kb-rose/30">
            ...
            <p className="text-kb-muted font-body text-[13px] leading-relaxed">
              Etkinlik sırasında paylaşacağın <strong className="text-kb-blush">story, reels ve içerikler</strong> seçilme şansını artırabilir 💫
            </p>
          </div>
          <div className="flex items-start gap-3 py-3 border-t border-kb-rose/30">
            <span className="text-kb-blush mt-0.5">📍</span>
            <p className="text-kb-muted font-body text-[13px] leading-relaxed">
              Etkinliği takip etmek için <strong className="text-kb-blush">{INSTAGRAM}</strong> hesabını takip et
            </p>
          </div>
        </div>
        ...
        <div className="w-full flex flex-col gap-3">
          ...
          <Link href="/" className="block">
            <button className="w-full py-3 text-[13px] text-kb-muted/60 font-body">Ana sayfaya dön</button>
          </Link>
        </div>

        <p className="mt-8 text-[11px] text-kb-muted/40 font-body leading-relaxed">
          Chae Lee K-Beauty Ambassador Program<br />Tüm hakları saklıdır.
        </p>
      </div>
    </main>
  );
